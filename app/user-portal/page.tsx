...
export default function UserPage() {
  ...
  async function saveProfile(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // SUCCESS:
      // 1. Update the local 'profile' state with the REAL data from the server
      // (This ensures your UI shows exactly what is in the database)
      setProfile(data.profile || form);

      // 2. Close the edit mode
      setIsEditing(false);
        ...
        <div className="afh-section galery-section w-[70%] md:w-[55%] max-md:w-full max-md:items-center -translate-y-[30px] section-padding flex flex-col gap-[40px]">
          ...
          {!onPublished && (
            <div className=" gallery-grid gap-[60px] grid-cols-2 max-lg:grid-cols-1 max-md:items-center font-primary text-[10px]">
              <div key="addNew" className="min-h-[300px] card card-hover bg-white flex flex-col items-center justify-center gap-[20px] border-[2px] border-afh-orange">
