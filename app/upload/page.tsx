...
export default function UploadPage() {
  ...
  const validate = () => {
    ...
    if (description.length > 1000) {
      addErrorMessage("description", "Description cannot exceed 1000 characters")
    }
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (!validTypes.includes(file.type)) {
      addErrorMessage("file", `Invalid file type. Please upload a valid ${selectedFileType || "media"} file.`)
      return false
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addErrorMessage("upload", error instanceof Error ? error.message : "Failed to upload artwork")
    } finally {
    ...
    <div className="min-h-screen bg-white px-4 sm:px-8 md:px-16 lg:px-32 py-12 sm:py-16 md:py-20">
      ...
      >
        ...
        <div className="w-full">
          ...
          />
          <label htmlFor="title" className={`block mt-2 text-sm font-light ${errors.title ? 'text-red-500' : 'text-gray-700'}`}>
            Artwork Title*
          </label>
        </div>
        ...
        <div className="w-full">
          <div ref={dropdownRef} className="relative">
            ...
            >
              <span className={formData.mediums.length === 0 ? "text-gray-400" : "text-gray-900"}>
